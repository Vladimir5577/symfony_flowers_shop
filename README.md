...
```
1. Symfony :80/:3000 → /api → PHP-FPM
2. Symfony :8080     → /admin, /login, /logout + EasyAdmin assets (`/bundles/*`) → PHP-FPM
3. Статика           → /uploads (файлы), /media/cache (ресайзы)
4. PHP-FPM           → обработка *.php
5. Vite              → HMR, исходники (только dev)
6. Фронтенд         → dist/ (prod) или Vite proxy (dev)
```

Добавить новый Symfony роут — добавить префикс в regex:

```nginx
location ~ ^/(api|новый_префикс)(/|$) {
```

Админка (`/admin`, `/login`, `/logout`) и EasyAdmin assets обрабатываются отдельным `server` на `:8080`.
